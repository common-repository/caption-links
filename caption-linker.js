...
jQuery(document).ready(function(){
  ...
  jQuery(document).ajaxComplete(function(){
    ...
    jQuery('label.setting.caption').append(link);
  });
  ...
  jQuery(document).on("click", '.caption-link button', function(){
    ...
    if ((url != null && url != 'http://') && text != null) {
      var tar=confirm("Open in same window?");
    }
    if ((url != null && url != 'http://') && text != null && tar == true) {
      box.insertAtCaret('<a href="' + url + '">' + text + '</a>');
    } else if ((url != null && url != 'http://') && text != null && tar == false) {
      box.insertAtCaret('<a href="' + url + '" target="_blank">' + text + '</a>');
    } else {
      // Error handle?
    }
    return false;
